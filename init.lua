...
minetest.register_globalstep(function(dtime)
    for _, player in ipairs(minetest.get_connected_players()) do
        ...
        player_states[player_name].footstep_timer = player_states[player_name].footstep_timer + dtime
        ...
        if (node.name == 'default:water_source' or node.name == 'default:water_flowing') and (vel.x ~= 0 or vel.y ~= 0 or vel.z ~= 0) then
            ...
            if player_states[player_name].splash_timer >= 2 then
                ...
                if head_node.name ~= 'default:water_source' then
                    if player_states[player_name].first_entry then
                        minetest.sound_play("splash3", {pos = pos, gain = 0.6, max_hear_distance = 16})
                        player_states[player_name].first_entry = false
                    end
            ...
            if player_states[player_name].entered_water and math.abs(vel.x) < 0.1 and math.abs(vel.y) < 0.1 and math.abs(vel.z) < 0.1 then
                ...
                player_states[player_name].first_entry = true
        ...
        if node.name == 'default:water_source' then
            ...
            if player_states[player_name].underwater_timer >= 5 then
